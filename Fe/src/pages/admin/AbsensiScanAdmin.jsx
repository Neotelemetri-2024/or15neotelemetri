...
import {
  ...
  FileText, Camera, CameraOff, RefreshCw,
} from "lucide-react";
import AdminLayout from "../../components/admin/LayoutAdmin";
import { getActivityById, scanAttendance, updateAttendance } from "../../services/attendanceService";
  ...
  const startCamera = useCallback(async (cameraId) => {
    ...
    try {
      ...
      if (cameraId && cameraId !== "environment" && cameraId !== "user") {
        ...
        videoConstraint = { facingMode: { ideal: cameraId || "environment" } };
      }
      ...
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        setErrorMsg("Izin kamera ditolak. Klik ikon kunci/kamera di address bar browser lalu izinkan akses kamera, kemudian refresh halaman.");
      } else if (err.name === "NotFoundError" || err.name === "DevicesNotFoundError") {
        setErrorMsg("Kamera tidak ditemukan. Pastikan kamera terhubung dan tidak dipakai aplikasi lain.");
      } else if (err.name === "NotReadableError" || err.name === "TrackStartError") {
        setErrorMsg("Kamera sedang dipakai aplikasi lain (Zoom, Meet, dll). Tutup aplikasi tersebut lalu coba lagi.");
      } else if (err.message === "Timeout") {
        setErrorMsg("Kamera tidak merespons. Coba ganti kamera atau refresh halaman.");
      } else {
    ...
    <div className="flex flex-col gap-3">
      ...
      {cameraList.length > 1 && (
        <div className="flex flex-col gap-1.5">
          <label className="text-gray-500 text-xs font-semibold">Pilih Kamera</label>
          <select
      ...
      <div className="relative w-full rounded-xl overflow-hidden bg-black"
        ...
        {status === "error" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/90 px-6">
            ...
            <p className="text-red-300 text-xs text-center leading-relaxed">{errorMsg}</p>
            <button
        ...
        {status === "ready" && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="relative w-52 h-52">
              ...
              {/* Scan line */}
              <div className="absolute left-2 right-2 h-0.5 animate-[scanline_2s_ease-in-out_infinite]"
                style={{ background: "#FF00FF", boxShadow: "0 0 8px #FF00FF", top: "50%" }} />
            </div>
...
export default function ScanAbsensiAdmin() {
  ...
  useEffect(() => { fetchActivity(); }, [activityId]);
  ...
  const handleScan = useCallback(async (userId) => {
    ...
    try {
      setScanLoading(true);
      setScanResult(null);
      const res = await scanAttendance({ userId: userId.trim(), activityId });
      const att = res.data;
      setScanResult({
        success: true,
        message: "Absensi berhasil dicatat!",
        name: att?.user?.profile?.fullName || att?.user?.email || "—",
      ...
      setScanResult({
        success: false,
        message: err?.response?.data?.message || "QR tidak valid atau user tidak ditemukan.",
      });
    } finally {
      ...
      <div className="min-h-screen flex flex-col gap-4 pt-10 md:pt-4 pb-10">
        ...
        {activity && (
          <div>
            ...
            <p className="text-white/50 text-xs mt-0.5">Deadline: {fmt(activity.deadline, "date")}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Hadir", value: stats.present, color: "#16a34a", bg: "#dcfce7" },
            { label: "Alfa",  value: stats.absent,  color: "#dc2626", bg: "#fee2e2" },
            { label: "Sakit", value: stats.sick,    color: "#d97706", bg: "#fef3c7" },
            { label: "Izin",  value: stats.excused, color: "#2563eb", bg: "#dbeafe" },
          ].map((s) => (
            <div key={s.label} className="rounded-xl px-3 py-3 flex flex-col items-center gap-1"
        ...
        {/* TAB SCAN */}
        {activeTab === "scan" && (
          <div className="flex flex-col gap-5 px-5 md:px-8 py-6"
            style={{ background: "white", borderRadius: "16px", boxShadow: "0 8px 48px rgba(120,0,200,0.18)" }}>

            <div>
              <p className="text-gray-800 font-bold text-sm">Scan QR Code User</p>
              <p className="text-gray-500 text-xs mt-1">Aktifkan kamera lalu arahkan ke QR Code user.</p>
            </div>
            ...
            {/* Placeholder */}
            {!cameraOn && (
              <div className="w-full rounded-xl flex flex-col items-center justify-center gap-3 py-10"
                style={{ background: "rgba(0,0,0,0.04)", border: "2px dashed rgba(0,0,0,0.12)" }}>
                <Camera size={36} className="text-gray-300" />
                <p className="text-gray-400 text-xs text-center">
                  Kamera belum aktif.<br />Tekan tombol di atas untuk mulai scan.
                </p>
              </div>
            )}
            ...
            {scanLoading && (
              <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-purple-50 border border-purple-200">
                ...
                <p className="text-xs text-purple-600 font-medium">Memproses absensi...</p>
              </div>
            )}
              ...
              <div className="flex items-start gap-3 px-4 py-3 rounded-xl"
                ...
                <div>
                  <p className="text-xs font-bold"
                    style={{ color: scanResult.success ? "#16a34a" : "#dc2626" }}>
                    {scanResult.message}
                  </p>
                  {scanResult.success && (
                    <p className="text-xs text-gray-600 mt-0.5">{scanResult.name} · {scanResult.nim}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <p className="text-gray-400 text-[11px] leading-relaxed">
              💡 <strong>Laptop:</strong> Pastikan tidak ada aplikasi lain yang memakai kamera (Zoom, Meet, dll).
              Akses via <code>localhost</code> atau <code>https://</code> — kamera tidak bisa diakses via <code>http://</code> selain localhost.
            </p>
          </div>
        )}
        ...
        {/* TAB LIST */}
        {activeTab === "list" && (
          <div style={{ background: "white", borderRadius: "16px", boxShadow: "0 8px 48px rgba(120,0,200,0.18)" }}>
            <div className="px-5 py-4 border-b" style={{ borderColor: "rgba(0,0,0,0.06)" }}>
              <p className="text-gray-800 font-bold text-sm">Daftar Kehadiran</p>
              <p className="text-gray-400 text-xs mt-0.5">{attendances.length} peserta terdaftar</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[560px]">
                <thead>
                  <tr style={{ borderBottom: "1.5px solid rgba(0,0,0,0.07)" }}>
                    {["No", "Nama", "NIM", "Check-in", "Status", "Action"].map((col) => (
                      <th key={col} className="p-4 text-xs font-bold text-gray-700 whitespace-nowrap"
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={att.id} className="hover:bg-purple-50 transition-colors"
                      style={{ borderBottom: i < attendances.length - 1 ? "1px solid rgba(0,0,0,0.05)" : "none" }}>
                      ...
                      <td className="p-4 text-gray-500 text-xs text-center">{att.user?.profile?.nim || "-"}</td>
                      <td className="p-4 text-gray-500 text-xs text-center whitespace-nowrap">
                        {att.checkInTime ? `${fmt(att.checkInTime, "date")} ${fmt(att.checkInTime, "time")}` : "-"}
                      </td>
                      <td className="p-4 text-center"><StatusBadge status={att.status} /></td>
                      <td className="p-4 text-center">
                        <button onClick={() => handleOpenEdit(att)}
      ...
      {editModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-6">
          <div className="bg-white rounded-2xl px-8 py-7 flex flex-col gap-5 max-w-sm w-full shadow-2xl">
            <div>
              <p className="text-gray-800 font-bold text-sm">Edit Kehadiran Manual</p>
              <p className="text-gray-500 text-xs mt-0.5">
                {editModal.user?.profile?.fullName || editModal.user?.email || "-"}
                {editModal.user?.profile?.nim ? ` · ${editModal.user.profile.nim}` : ""}
              </p>
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-gray-700 font-semibold text-xs">Catatan (opsional)</label>
              <textarea rows={3} placeholder="Contoh: Sakit dengan surat dokter"
