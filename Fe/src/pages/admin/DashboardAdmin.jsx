...
function HorizontalBar({ label, value, total, color, rank }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-gray-500 text-[10px] w-4 text-right shrink-0">
        {rank}
      </span>
...
function StatistikModal({ users, programStudiMap, onClose }) {
  ...
  const fakultasData = Object.entries(fakultasCount).sort(
    (a, b) => b[1] - a[1],
  );
  ...
  const angkatanData = Object.entries(angkatanCount).sort((a, b) =>
      ...
      >
        ...
        >
          <span className="text-[10px] text-gray-400">
            {activeTab === "prodi" &&
            prodiData.length < Object.keys(prodiCount).length
...
export default function DashboardAdmin() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const prodiMap = {};
        (prodiRes.data ?? []).forEach((ps) => {
          prodiMap[ps.id] = ps.name;
        });
  ...
  const filteredUsers = users.filter((u) => {
    ...
    if (search) {
      ...
      return (
        ...
        getProgramStudiName(u.profile?.studyProgramId)
          ?.toLowerCase()
          .includes(q) ||
  ...
  const handleExportCurrentDivision = () => {
    ...
    XLSX.writeFile(
      wb,
      `Data_Pendaftar_${divName}_${new Date().toLocaleDateString("id-ID").replace(/\//g, "-")}.xlsx`,
    );
  ...
  const handleExportAllDivisions = () => {
    ...
    divisions.forEach((div) => {
      const divUsers = users
        .filter((u) => {
          ...
          return (subDivisionMap[div.id] || []).some(
            (s) => s.id === u.profile.subDivisionId,
          );
        })
        .sort((a, b) =>
          !!a.profile?.subDivisionId === !!b.profile?.subDivisionId
            ? 0
            : !!a.profile?.subDivisionId
              ? -1
              : 1,
        );
    ...
    XLSX.writeFile(
      wb,
      `Data_Semua_Divisi_${new Date().toLocaleDateString("id-ID").replace(/\//g, "-")}.xlsx`,
    );
    setExportMenuOpen(false);
  };
