...
export default function OrganisasiSection() {
  ...
  const currentDivision = organisasi[0]; // tetap pakai organisasi pertama
  const allSubs = currentDivision.subDivisions;

  // Bagi subDivisions jadi chunks of 3
  const chunks = [];
  ...
  const visibleCards = chunks[pageIndex]; // 3 card yang tampil
  ...
  const changePage = (direction) => {
    ...
    // Setelah flip selesai → ganti halaman
    setTimeout(
      () => {
        setPageIndex((prev) => {
  ...
  return (
    <section id="division" className="relative ">
      {/* ================= BACKGROUND ================= */}
